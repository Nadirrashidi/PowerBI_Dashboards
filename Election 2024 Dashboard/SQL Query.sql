...
select count(Constituency_ID) as 'Total Seats'from constituencywise_results

-- What is the total number of seats available for elections in each state

SELECT s.State ,count(cr.Constituency_ID) as "Total setas"
FROM States s
JOIN Statewise_Results sr ON sr.State_ID = s.State_ID
join constituencywise_results cr on cr.Parliament_Constituency=sr.Parliament_Constituency
GROUP BY s.State
ORDER BY [Total setas] asc
